<template>
  <div class="content">
    <div class="container-fluid" >
      <div class="row" >
        <div class="col-xl-6 col-md-6" v-for="(park,key) in showpark" v-if="park.status == 'On'">
          <stats-card >
            ...
            <div slot="content">
              ...
              <h8 class="card-title" style="color:green;"><i class="fas fa-check-circle" style="color:green;"></i> :  {{ park.count }}</h8 ><br>
            ...
            <div slot="footer" >
              <i class="fa fa-refreshis-center"></i>
                <p>{{ todayDate }}</p>
                <a class="btn " @click="mapnew(key,park.location)">เส้นทาง </a>
                  <!-- <button class="btn"
                  @click="resev(park.name,park.Total)">
                  จอง
                  </button> -->
                    <button class="btn" @click="Sesevpark()"> จอง
                    </button>
            </div>
          </stats-card>
          <div class="col-12">
            <b-modal :active.sync="isImageModalActive">
              ...
              <div  v-for="(n,index) in showpark" >
                <table v-for="(v,index) in n" v-if="index == parkname">
                  <tr v-for="(ve,index,key) in v" >
                    <!-- <center><td><a class="button is-success" @click="isResaver(index+1,key,index)" >จองที่จอดรถที่ {{index+1 }} <br>
                    </a></td></center> -->
                    <br>
                    <center><a class="btn is-success" @click="isResaver(index+1,key,index,ve)">ช่องจอดรถที่ {{ key+1 }}
                    </a>
                    <i class="fas fa-exclamation-circle" @click="warn(index,key)"></i></center>
                  </tr>
                </table>
              </div>
              <br>
                  ...
                  <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <option value="55" v-if="selectedtime == 14 && parseFloat(todayhous,10).toFixed(0)+ '.' + todayminit  < 14.55">55 น.</option>
                            ...
                            <option value="00" v-if="selectedtime == 15 && parseFloat(todayhous,10).toFixed(0)+ '.' + todayminit < 15.05 "> 00 น. </option>
              ...
              <br>
                <div v-for="(n,index) in showpark" >
                  <table v-for="(v,index) in n" v-if="index == parkname">
                    ...
                    <tr v-for="(ve,index,key) in v" v-if="indexwarn == index">
                      <td v-for="(vee,key,index) in ve">
                        {{ vee.start }} : {{ vee.End }}
                      </td>
                      <td  v-for="(vee,key) in ve"> {{ vee.name }}                       
                      </td>
                    </tr>
                  </table>
                </div>
        ...
        <div class="col-xl-6 col-md-6" v-for="park in showpark" v-if="park.status == 'Off'">
          <stats-card>
            ...
            <div slot="footer" >
              <i class="fa fa-refreshis-center"></i>
                <p>{{ todayDate }}</p>
                <a class="btn " @click="mapnew(key,park.location)">เส้นทางสำรอง</a>
                <button class="btn" @click="Sesevpark()">ที่จอดรถสำรอง
                </button>
            </div>
          </stats-card>
        </div>
...
<script>
 ...
 mapnew (key,lo) {
  window.open("https://www.google.com/maps/place/"+lo)
 },
 Sesevpark () {
  this.$router.replace('/Admin/Resevpark')
 },
 warn(index,key) {
    ...
    commitReser () {
      ...
      for (var variable in this.showuser) {
        ...
        for(var parker in this.showpark){
            if(this.showpark[parker].name === this.parkname){
              console.log("เข้าทำ loop 703")
              console.log(this.showpark[parker])
              firebase.database().ref('Park').child(parker).child(this.parkname).child(this.index).push(tmp)
